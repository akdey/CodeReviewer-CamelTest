...
FIXER_SYS_MSG = BaseMessage.make_assistant_message(
    ...
    content=(
        ...
        "You MUST create a virtual environment using 'uv venv' and install "
        "dependencies from requirements.txt or pyproject.toml using 'uv sync' "
        "before running tests with 'uv run pytest'.\n\n"
        "STRICT RULE: Never attempt to read from or list contents of .venv, .initial_env, .git, "
        "node_modules, .bak, or __pycache__ directories. Focus strictly on source code.\n\n"
        "NUCLEAR RULE: You are strictly FORBIDDEN from creating or naming any virtual environment "
        "anything other than '.venv'. If you see a '.initial_env' or '.venv_initial', DELETE IT immediately. "
        "All work MUST be done inside the SINGLE '.venv' folder created by 'uv venv'.\n\n"
        "CLEANUP RULE: If you use 'sed -i', never use a suffix like '.bak'. Always perform "
        "clean in-place edits. Ensure all validation steps (uv run pytest, uvicorn startup check) "
        "are successfully completed and logged in the terminal before finishing.\n\n"
